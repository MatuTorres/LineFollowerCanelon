...
Empieza inicializando los pines, nada que aclarar acá.
...
$E(t)$ es el error en el tiempo, tratar a el error como una función nos va a servir el el cálculo del PID, más adelante
...
- ```VelMax``` es la velocidad máxima
- ```currentTime```, ```previousTime``` y ```elapsedTime``` guardan lo que su nombre indica, usados en el cálculo de la integral y la derivada del error.
...
Aquí una de las funciones más importantes, la que se ocupa de la **calibración**, está enciende el ```led```, integrado en el arduino, para indicar que el auto está en proceso de calibración.

Durante la calibración, se guardan los **valores máximos y mínimos** que toman los sensores para luego saber la diferencia entre blanco y negro.

Al terminar de calibrar, apaga el ```led``` indicando que se terminó de calibrar. Y luego inicia la serial para mostrar en esta los valores mínimos y máximos que tomó cada sensor (función ```printCalibration```).
